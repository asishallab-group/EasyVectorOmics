...
#' Check if a paralog is active in a bit mask
mask_check_state <- function(bit_mask, i_paralog) {
  if (!is.integer(bit_mask)) {
    stop("bit_mask must be an integer vector")
  }
  if (!is.integer(i_paralog) || length(i_paralog) != 1) {
    stop("i_paralog must be a single integer")
  }

  n_mask_chunks <- as.integer(length(bit_mask))
  state <- logical(1)

  res <- .Fortran("mask_check_state_r",
                  bit_mask = bit_mask,
                  n_mask_chunks = n_mask_chunks,
                  i_paralog = as.integer(i_paralog),
                  state = state)

  res$state
}

#' Compute number of 32-bit chunks needed for paralog mask
mask_chunk_count <- function(n_paralogs) {
  if (!is.integer(n_paralogs) || length(n_paralogs) != 1) {
    stop("n_paralogs must be a single integer")
  }

  count <- integer(1)

  res <- .Fortran("mask_chunk_count_r",
                  n_paralogs = as.integer(n_paralogs),
                  count = count)

  res$count
}

#' Calculate work array size for paralog subset analysis
calc_work_arr_paralog_subsets_size <- function(max_subset_size, n_paralogs, filtered_paralogs_mask) {
  if (!is.integer(max_subset_size) || length(max_subset_size) != 1) {
    stop("max_subset_size must be a single integer")
  }
  if (!is.integer(n_paralogs) || length(n_paralogs) != 1) {
    stop("n_paralogs must be a single integer")
  }
  if (!is.integer(filtered_paralogs_mask)) {
    stop("filtered_paralogs_mask must be an integer vector")
  }

  n_mask_chunks <- as.integer(length(filtered_paralogs_mask))
  work_array_size <- integer(1)
  ierr <- integer(1)

  res <- .Fortran("calc_work_arr_paralog_subsets_size_r",
  ...
  list(work_array_size = res$work_array_size,
       max_subset_size = res$max_subset_size)
}

#' Filter paralogs by pattern dosage effect
filter_paralogs_by_pattern_dosage_effect <- function(paralog_angles, threshold) {
  if (!is.numeric(paralog_angles)) {
    stop("paralog_angles must be a numeric vector")
  }
  if (!is.numeric(threshold) || length(threshold) != 1) {
    stop("threshold must be a single numeric value")
  }

  n_paralogs <- as.integer(length(paralog_angles))
  n_mask_chunks <- as.integer(ceiling(n_paralogs / 32L))
  mask <- integer(n_mask_chunks)
  ierr <- integer(1)

  res <- .Fortran("filter_paralogs_by_pattern_dosage_effect_r",
                  paralog_angles = as.double(paralog_angles),
...
filter_paralogs_by_pattern_subfunctionalization <- function(paralog_angles, threshold) {
  if (!is.numeric(paralog_angles)) {
    stop("paralog_angles must be a numeric vector")
  }
  if (!is.numeric(threshold) || length(threshold) != 1) {
  ...
  res <- .Fortran("filter_paralogs_by_pattern_subfunctionalization_r",
                  paralog_angles = as.double(paralog_angles),
                  threshold = as.double(threshold),
                  n_paralogs = n_paralogs,
                  mask = mask,
...
detect_subfunctionalization <- function(ancestor, paralogs, rdi_threshold,
  ...
  if (!is.numeric(rdi_threshold) || length(rdi_threshold) != 1) stop("rdi_threshold must be a single numeric value")
  ...
  if (!is.numeric(paralog_norms)) stop("paralog_norms must be a numeric vector")
  if (!is.integer(sorted_paralog_norms_perm)) stop("sorted_paralog_norms_perm must be an integer vector")

  n_dims <- as.integer(length(ancestor))
  ...
  temp_work_array <- numeric(n_paralogs)
  n_results <- integer(1)
  ierr <- integer(1)

  res <- .Fortran("detect_subfunctionalization_r",
                  ancestor = as.double(ancestor),
                  paralogs = as.double(paralogs),
                  n_paralogs = n_paralogs,
                  n_dims = n_dims,
                  rdi_threshold = as.double(rdi_threshold),
                  filtered_paralogs_mask = filtered_paralogs_mask,
                  n_mask_chunks = n_mask_chunks,
                  n_results = n_results,
                  ...
                  paralog_norms = as.double(paralog_norms),
...
detect_dosage_effect <- function(ancestor, paralogs, filtered_paralogs_mask,
  ...
  res <- .Fortran("detect_dosage_effect_r",
                  ancestor = as.double(ancestor),
                  paralogs = as.double(paralogs),
                  n_paralogs = n_paralogs,
                  n_dims = n_dims,
                  ...
                  temp_paralog_vector = temp_paralog_vector,
                  ierr = ierr,
                  max_angle = as.double(max_angle),
                  gain_gamma = as.double(gain_gamma))
