...
relative_axes_changes_from_shift_vector <- function(vec) {
  ...
  res <- .Fortran('relative_axes_changes_from_shift_vector_r',
                  ...
                  err = as.integer(0))
  # Only stop for error code 201 (invalid input)
  if (!is.null(res$err) && res$err == 201) {
    stop('Invalid input provided to relative_axes_changes_from_shift_vector')
  }
  return(res$contrib)
}
...
relative_axes_expression_from_expression_vector <- function(vec) {
  ...
  # Only stop for error code 201 (invalid input)
  if (!is.null(res$err) && res$err == 201) {
    stop('Invalid input provided to relative_axes_expression_from_expression_vector')
  }
  return(res$contrib)
}
...
omics_vector_RAP_projection <- function(vecs, vecs_selection_mask, axes_selection_mask) {
  ...
  res <- .Fortran("omics_vector_RAP_projection_r",
                  ...
                  ierr = ierr)
  if (!is.null(res$ierr) && res$ierr != 0) {
    tox_errors(res$ierr)
  }
