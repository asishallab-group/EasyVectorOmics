...
pdf('figure_1a_medical.pdf', width = 8, height = 6)
...
orth_1_pts <- map_time_to_coord(orth_1, y_base_series)
orth_2_pts <- map_time_to_coord(orth_2, y_base_series)
mean_pts  <- map_time_to_coord(mean, y_base_series)

diff_pts <- matrix(c(
  0.8, y_base_series + 1.2,
  1.3, y_base_series + 1.4,
  2.0, y_base_series + 1.6,
  2.5, y_base_series + 1.5,
  3, y_base_series + 1.9
), ncol = 2, byrow = TRUE)
...
for (i in 1:nrow(mean_pts)) {
  arrows(mean_pts[i, 1], mean_pts[i, 2],
         ...
         length = 0.09, col = colors[[3]], lwd = 2.2)
}
...
for (i in 1:nrow(mean_pts)) {
  ...
  y_coord <- (mean_pts[i, 2] + diff_pts[i, 2]) / 2 + 0.12
