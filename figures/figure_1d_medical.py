...
def draw_smooth_line(ax, points, k=2, periodic=False, label="", label_move_rel=(0, 0), color="#377EB8", fontsize=14):
    ...
    ax.text(x_smooth[mid_index] + label_move_rel[0], y_smooth[mid_index] + label_move_rel[1], label, color=color, ha="center", fontsize=fontsize)
...
def plot_1d():
    ...
    def standalone_arrows():
        ...
        high_impact = (
            arrow(18, 46.5, 7, -1, length=14),
            arrow(10, 24.5, 17, length=13),
            arrow(25, 39.5, 22, -1, length=11),
        )
        return (
            *high_impact,
            random_arrow(7, 75, 9),
    ...
    def left_blue(color="#004C99"):
        ...
        draw_smooth_line(ax, route_left_line, color=color, label="Class -", label_move_rel=(-5, -15))


        return arrows

    # second from left, the one below the circle with two arrows
    def right_blue(color="#66B2FF"):
        arrows = (
            arrow(56, 16, 52),
            arrow(56, 10, 47),
        ...
        draw_smooth_line(ax, route_left_line, color="#377EB8", label="DOI\nsubfields", label_move_rel=(11, 22.5), fontsize=12)

        draw_smooth_line(ax, route_left_line, color=color, label="Class +", label_move_rel=(-15, -5))
    ...
    def chaotic(color="blue"):
        ...
        draw_smooth_line(ax, route, periodic=True, color=color, label="Inflammatory response", label_move_rel=(0, 35))
    ...
    # Línea vertical con "cabezas" tipo |------| (rotado)
    x_pos = 47  # posición X entre los subcampos azules
    y_start = 28  # ajusta según ubicación inferior
    y_end = 48    # ajusta según ubicación superior

    # Línea central vertical
    ax.plot([x_pos, x_pos], [y_start, y_end], color='#377EB8', linewidth=1.5)

    # "Cabezas" horizontales (como los palitos de |)
    head_width = 3  # longitud de los cabezales horizontales
    ax.plot([x_pos - head_width/2, x_pos + head_width/2], [y_start, y_start], color='#377EB8', linewidth=1.5)
    ax.plot([x_pos - head_width/2, x_pos + head_width/2], [y_end, y_end], color='#377EB8', linewidth=1.5)

    # Texto de distancia (opcional)
    ax.text(x_pos - 2.5, (y_start + y_end)/2, "d", ha="right", va="center", fontsize=14, color='#377EB8')
