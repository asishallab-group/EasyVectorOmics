...
contains
  ...
  ! ============================================================
  ! Thin wrappers with descriptive names for NetLib LOESS subroutines
  ! ============================================================

  !> Initializes workspace arrays and performs LOESS decomposition (wraps NetLib lowesd).
  subroutine loess_decomposition(mode, int_workspace, int_workspace_size, real_workspace_size, real_workspace, dim, n, span, degree, nvmax, setlf)
    integer(int32), intent(in)    :: mode, int_workspace_size, real_workspace_size, dim, n, degree, nvmax
    real(real64),   intent(in)    :: span
    logical,        intent(in)    :: setlf
    integer(int32), intent(inout) :: int_workspace(*)
    real(real64),   intent(inout) :: real_workspace(*)
    call lowesd(mode, int_workspace, int_workspace_size, real_workspace_size, real_workspace, dim, n, span, degree, nvmax, setlf)
  end subroutine loess_decomposition

  !> Fits the LOESS model and computes hat matrix diagonal (wraps NetLib lowesb).
  subroutine loess_fitting(x, y, w, diagl, infl, int_workspace, int_workspace_size, real_workspace_size, real_workspace)
    real(real64),   intent(in)    :: x(*), y(*), w(*)
    real(real64),   intent(out)   :: diagl(*)
    logical,        intent(in)    :: infl
    integer(int32), intent(in)    :: int_workspace(*), int_workspace_size, real_workspace_size
    real(real64),   intent(inout) :: real_workspace(*)
    call lowesb(x, y, w, diagl, infl, int_workspace, int_workspace_size, real_workspace_size, real_workspace)
  end subroutine loess_fitting

  !> Evaluates the fitted LOESS model at given x-points (wraps NetLib lowese).
  subroutine loess_evaluation(int_workspace, int_workspace_size, real_workspace_size, real_workspace, n, eval_points, fitted_values)
    integer(int32), intent(inout) :: int_workspace(*)
    integer(int32), intent(in)    :: int_workspace_size, real_workspace_size, n
    real(real64),   intent(inout) :: real_workspace(*)
    real(real64),   intent(in)    :: eval_points(*)
    real(real64),   intent(out)   :: fitted_values(*)
    call lowese(int_workspace, int_workspace_size, real_workspace_size, real_workspace, n, eval_points, fitted_values)
  end subroutine loess_evaluation

  !> Computes bisquare robust weights from residuals (wraps NetLib lowesw).
  subroutine loess_robust_weights(residuals, n, robust_weights, permutation_indices)
    real(real64),   intent(in)  :: residuals(*)
    integer(int32), intent(in)  :: n
    real(real64),   intent(out) :: robust_weights(*)
    integer(int32), intent(out) :: permutation_indices(*)
    call lowesw(residuals, n, robust_weights, permutation_indices)
  end subroutine loess_robust_weights
  ...
  subroutine tox_loess_required_workspace(d, nvmax, int_workspace_size, real_workspace_size, setlf)
    ...
    if (setlf) int_workspace_size = int_workspace_size + nvmax
    real_workspace_size = 100 + (10 * d + 20) * nvmax
    if (setlf) real_workspace_size = real_workspace_size + (d + 1) * nvmax
    int_workspace_size = max(10000_int32, int_workspace_size)
    real_workspace_size  = max(100000_int32, real_workspace_size)
  ...
  subroutine loess_fit_plain(n, x, y, w, eval_points, span, degree, nvmax, infl, setlf, int_workspace, int_workspace_size, real_workspace, real_workspace_size, diagl, fitted_values, ierr)
    ...
    if (int_workspace_size < 10000 .or. real_workspace_size < 100000) then
