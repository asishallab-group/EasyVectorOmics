...
\subsection{Input Data and Initial Representation}
...
Before constructing the gene expression vector space, raw count data must be normalized to allow for meaningful geometric comparison. In Tensor Omics, the normalization pipeline is deliberately simple and interpretable, designed to retain the biological semantics of the vector structure.

\subsection{Standard Deviation Scaling (No Centering)}

Let each gene \( g \) be associated with an expression vector \( \mathbf{x}_g = [x_{g,1}, x_{g,2}, \dots, x_{g,n}] \), where each component corresponds to a sample (e.g., different tissues, replicates, time points, or stress conditions).

We normalize each gene's expression vector independently by dividing all components by its standard deviation:

\[
\hat{x}_{g,i} = \frac{x_{g,i}}{\sigma_g}
\quad \text{where} \quad 
\sigma_g = \sqrt{\frac{1}{n} \sum_{j=1}^n x_{g,j}^2}
\]

\textbf{Important:} We do not subtract the mean. This avoids removing the overall direction of the expression vector, which carries biological meaning—such as tissue preference or dominant condition.

\paragraph{Example:}  
Suppose a gene is expressed in three replicates each of \texttt{root}, \texttt{stem}, and \texttt{leaf}, with raw counts:

\[
\mathbf{x}_g = [120, 135, 110, 40, 50, 45, 5, 10, 12]
\]

After standard deviation normalization, we get:

\[
\hat{\mathbf{x}}_g = \left[ \frac{120}{\sigma_g}, \frac{135}{\sigma_g}, \dots, \frac{12}{\sigma_g} \right]
\]

This ensures that all vectors reside on comparable scales, while preserving angular relationships and magnitude differences needed for geometric and semantic analysis.

\subsection{Quantile Normalization and Log-Transformation}

\textbf{Quantile normalization} across samples is optional and typically used to ensure comparable distributions across conditions—especially useful when comparing different omics types. However, care must be taken when integrating transcriptomics with proteomics or metabolomics.

\textbf{Log-transformation} is applied as:

\[
x' = \log(1 + x)
\]

This transformation reduces the impact of outliers and brings expression values closer to normality without distorting their relative magnitudes when \( x \) is large.


\subsection{Construction of the Gene Expression Space}
...
\subsection{Gene Family Centroids}
...
\subsection{The Space Diagonal and Expression Neutrality}
...
\subsection{Angular Deviation from the Diagonal}
...
\section{Relative Axes Expression: Clock Plots and Semantic Rotation}

\subsection{Projection onto the Diagonal-Orthogonal Plane}
...
\subsection{Clock Hand Vectors and Angle $\phi$}
...
\subsection{Difference Vector and Unit Difference Vector}
...
\subsection{Biological Interpretations and Use Cases}
...
\section{Meta-Data Filtering and Comparative Analysis}

\subsection{Subsetting the Semantic Vector Space}
...
\subsection{Distributional Comparison of Expression Differences}
...
\subsection{Interpretation of Semantic Flow}
...
\subsection{Example Applications from the Proposal}
...
\section{Software Architecture and Implementation Plan}

\subsection{Language and Framework Choice}
...
\subsection{Data Structure and Serialization}
...
\subsection{Workbench for Interactive Visualization}
...
\subsection{Patching and Reproducibility}
...
\section{Summary and Evaluation of Tensor Omics}
