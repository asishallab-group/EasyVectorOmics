...
module asserts
  ...
  public :: assert_sorted_real, assert_same_length_1d, assert_string_equal
...
contains
  ...
  ! Assert that two 1D arrays have the same length.
  subroutine assert_same_length_1d(len1, len2, msg)
    !| len1: Length of the first 1D array.
    integer(int32), intent(in) :: len1
    !| len2: Length of the second 1D array.
    integer(int32), intent(in) :: len2
    character(*), intent(in) :: msg
    if (len1 /= len2) then
        write(error_unit,*) "ASSERTION FAILED: ", trim(msg), " (lengths differ: ", len1, " vs ", len2, ")"
        stop 1
    end if
  end subroutine
  ...
  ! Assert that two 1D real arrays are close within relative and absolute tolerance.
  ! NOTE: This assumes both 1D arrays have the same length, `n`.
