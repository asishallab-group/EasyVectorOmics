...
contains
  ...
  subroutine test_identical_vectors_2d()
    ...
    integer :: selected_axes(3)
    
    v1 = [1.0_real64, 0.0_real64]
    v2 = [1.0_real64, 0.0_real64]
    selected_axes = [1, 2, 1]  ! Ignored for 2D
    
    call clock_hand_angle_between_vectors(v1, v2, 2, signed_angle, selected_axes)
  ...
  subroutine test_45_degree_rotation_2d()
    ...
    v1 = [1.0_real64, 0.0_real64]
    v2 = [sqrt(2.0_real64)/2.0_real64, sqrt(2.0_real64)/2.0_real64]  ! 45 degrees
  ...
  subroutine test_clockwise_vs_counterclockwise_2d()
    ...
    v1 = [1.0_real64, 0.0_real64]
    v2_ccw = [0.0_real64, 1.0_real64]   ! 90° counterclockwise
    v2_cw = [0.0_real64, -1.0_real64]   ! 90° clockwise
    selected_axes = [1, 2, 1]  ! Ignored for 2D
    
    call clock_hand_angle_between_vectors(v1, v2_ccw, 2, angle_ccw, selected_axes)
    call clock_hand_angle_between_vectors(v1, v2_cw, 2, angle_cw, selected_axes)
  ...
  subroutine test_identical_vectors_3d()
    ...
    selected_axes = [1, 2, 3]  ! Ignored for 3D
    
    call clock_hand_angle_between_vectors(v1, v2, 3, signed_angle, selected_axes)
  ...
  subroutine test_high_dimensional_basic()
    ...
    selected_axes = [1, 2, 3]  ! Use first 3 dimensions for orientation
    
    call clock_hand_angle_between_vectors(v1, v2, 5, signed_angle, selected_axes)
  ...
  subroutine test_high_dimensional_selected_axes()
    ...
    selected_axes = [3, 5, 1]  ! Use dimensions 3, 5, 1 for orientation
    
    call clock_hand_angle_between_vectors(v1, v2, 7, signed_angle, selected_axes)
  ...
  subroutine test_zero_vectors()
    ...
    call clock_hand_angle_between_vectors(v1, v2, 3, signed_angle, selected_axes)
    ! Should handle gracefully - likely NaN or some defined behavior
  ...
  subroutine test_multiple_pairs_shift_vectors()
    ...
    origins(:, 1) = [1.0_real64, 0.0_real64]
    targets(:, 1) = [0.0_real64, 1.0_real64]  ! 90° CCW
    
    origins(:, 2) = [1.0_real64, 0.0_real64]
    targets(:, 2) = [-1.0_real64, 0.0_real64]  ! 180°
    
    origins(:, 3) = [1.0_real64, 0.0_real64]
    targets(:, 3) = [0.0_real64, -1.0_real64]  ! 90° CW
    
    vecs_selection_mask = [.true., .true., .true.]
    selected_axes = [1, 2, 1]
    
    call clock_hand_angles_for_shift_vectors(origins, targets, 2, 3, vecs_selection_mask, 3, &
                                           selected_axes, signed_angles)
  ...
  subroutine test_shift_vectors_with_selection_mask()
    ...
    origins(:, 2) = [1.0_real64, 0.0_real64]
    targets(:, 2) = [-1.0_real64, 0.0_real64]  ! Should be selected
    
    origins(:, 3) = [1.0_real64, 0.0_real64]
    targets(:, 3) = [0.0_real64, -1.0_real64]
    
    origins(:, 4) = [1.0_real64, 0.0_real64]
    targets(:, 4) = [sqrt(2.0_real64)/2.0_real64, sqrt(2.0_real64)/2.0_real64]  ! Should be selected
    
    vecs_selection_mask = [.false., .true., .false., .true.]
    selected_axes = [1, 2, 1]
    
    call clock_hand_angles_for_shift_vectors(origins, targets, 2, 4, vecs_selection_mask, 2, &
                                           selected_axes, signed_angles)
  ...
  subroutine test_invalid_selected_axes()
    ...
    v1 = [1.0_real64, 0.0_real64, 0.0_real64, 0.0_real64, 0.0_real64]
    v2 = [0.0_real64, 1.0_real64, 0.0_real64, 0.0_real64, 0.0_real64]
    selected_axes = [1, 1, 1]  ! All same indices
    
    call clock_hand_angle_between_vectors(v1, v2, 5, signed_angle, selected_axes)
    ! Should handle gracefully - may give zero or some defined result
  ...
  subroutine test_out_of_bounds_selected_axes()
    ...
    v1 = [1.0_real64, 0.0_real64, 0.0_real64]
    v2 = [0.0_real64, 1.0_real64, 0.0_real64]
    selected_axes = [1, 2, 5]  ! Index 5 is out of bounds for 3D vector
    
    ! This should be caught by bounds checking or handled gracefully
    call clock_hand_angle_between_vectors(v1, v2, 3, signed_angle, selected_axes)
  ...
  subroutine test_performance_large_scale()
    ...
    call clock_hand_angles_for_shift_vectors(origins, targets, n_dims, n_vecs, vecs_selection_mask, &
                                           n_vecs, selected_axes, signed_angles)
    
    ! Basic sanity check - all angles should be in valid range
    do i = 1, n_vecs
  ...
  subroutine test_consistency_between_functions()
    ...
    call clock_hand_angle_between_vectors(v1, v2, 3, single_angle, selected_axes)
    ...
    call clock_hand_angles_for_shift_vectors(origins, targets, 3, 1, vecs_selection_mask, 1, &
                                           selected_axes, batch_angles)
  ...
  subroutine test_mathematical_properties()
    ...
    call clock_hand_angle_between_vectors(v1, v2, 2, angle_12, selected_axes)
    call clock_hand_angle_between_vectors(v2, v1, 2, angle_21, selected_axes)
    
    ! Angles should be opposite in sign (anti-commutative for signed angles)
