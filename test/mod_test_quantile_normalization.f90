...
contains
  ...
  subroutine test_qn_single_row()
    ...
    row_mean = sum(mat(1,:)) / ncol  ! = 2.5
    expected(1,:) = row_mean  ! All values should become the mean
    
    call quantile_normalization_r(nrow, ncol, mat, result, temp_col, rank_means, perm, stack_left, stack_right, max_stack)
    
    ! For single row, all values should become the mean of the row
  ...
  subroutine test_qn_single_column()
    ...
    expected = mat  ! Single column should preserve values (no normalization across samples)
    
    call quantile_normalization_r(nrow, ncol, mat, result, temp_col, rank_means, perm, stack_left, stack_right, max_stack)
    
    ! Single column should remain unchanged (no other columns to normalize against)
