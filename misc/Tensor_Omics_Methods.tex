...
\subsection{Concept of the Subfield Effect}

In Tensor Omics, a \textit{subfield} is a subset of vectors 
\(\{\vec{v}_i\}_{i=1}^{m}\) extracted from an omics vector space 
(e.g. expression vectors or shift vectors) that share semantic proximity 
or biological annotation (e.g. ``photosynthesis'', ``inflammatory response'').
The collective influence of such a subfield can be described as the cumulative 
effect that all its vectors exert on a \textit{virtual particle} positioned at 
the center of the subfield. 

This analogy allows the use of linear algebraic descriptors that quantify 
the structure, energy, and dynamics of a subfield, independent of its 
dimensionality or biological origin.

We represent the subfield as a matrix
\[
M = [\, \vec{v}_1, \vec{v}_2, \ldots, \vec{v}_m \,] \in \mathbb{R}^{n \times m},
\]
where each column corresponds to an individual vector in the subfield.  
The cumulative covariance structure of the field is then given by
\[
C = M M^{\mathsf{T}}.
\]
This covariance captures both the magnitude and directional coherence of 
vectors in the subfield.
...
rotation --- form the \textbf{Tensor Omics Field Descriptors (TOXFD)}.

\subsection{Vector Sets versus Vector Fields}

A \textit{set of vectors} (e.g. all expression vectors of a gene family) 
is described only by the vectors themselves and their distribution in space.
In contrast, a \textit{vector field} additionally associates each vector 
with a coordinate (position), such as the centroid of a gene family or 
a point in tissue-expression space:
\[
\mathcal{F} = \{\, (\vec{x}_i, \vec{v}_i) \,|\, \vec{x}_i, \vec{v}_i \in \mathbb{R}^n \,\}.
\]

For such fields, TOXFDs can be computed in two complementary ways:

\begin{itemize}
  \item On the \textbf{coordinates} \(\vec{x}_i\):  
  ...
  \item On the \textbf{shift vectors} \(\vec{v}_i\):  
...
\textbf{Examples.}  
In transcriptomics, coordinates represent gene families in expression space, 
while shift vectors represent expression changes (e.g. stress response).  
In physics, coordinates might represent spatial positions, and shift vectors 
might represent wind velocity or magnetic field direction.

\subsection{Tracing a Field along its Semantic Axis}

Assume a subfield has an associated \textit{semantic axis}
\(\vec{d}_{\text{sem}}\) (see Section~X.X for its definition), 
